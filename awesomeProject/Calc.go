...
func changeRoma(a string, b string, c string) string {
	...
	if a2 > 0 && c2 > 0 {
		...
		default:
			strconv.Itoa(t)
			t := "Не корректный оператор, повторите ввод"
			return t
		}
	} else {
		a2, _ = strconv.Atoi(a)
		c2, _ = strconv.Atoi(c)
		if a2 == 0 || c2 == 0 {
			t := "Нельзя использовать Римские и Арабские числа в одной операции, а также использовать 0 или дробные числа, повторите ввод"
			return t
		}
		if a2 > 10 || c2 > 10 {
			t := "Aргументы должны быть кратны или меньше 10, повторите ввод"
